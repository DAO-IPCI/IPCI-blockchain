    ...
    NumberFor<TBlock>: sc_finality_grandpa::BlockNumberOps,
{
    ...
    if matches!(config.role, Role::Light) {
        ...
        generate_request_response_config(protocol_id)
    } else {
        // Allow both outgoing and incoming requests.
        let (handler, request_response_config) =
            GrandpaWarpSyncRequestHandler::new(protocol_id, backend);
...
impl<TBlock: BlockT, TBackend: Backend<TBlock>> GrandpaWarpSyncRequestHandler<TBackend, TBlock> {
        ...
        NumberFor<TBlock>: sc_finality_grandpa::BlockNumberOps,
    {
        ...
        let response = sc_finality_grandpa::prove_warp_sync(
            ...
            request.begin,
            Some(WARP_SYNC_FRAGMENTS_LIMIT),
            Some(&mut cache),
        )?;
        ...
        NumberFor<TBlock>: sc_finality_grandpa::BlockNumberOps,
    {
        while let Some(request) = self.request_receiver.next().await {
            ...
            match self.handle_request(payload, pending_response) {
                Ok(()) => debug!(
                    target: LOG_TARGET,
                    "Handled grandpa warp sync request from {}.", peer
                ),
                Err(e) => debug!(
                    target: LOG_TARGET,
